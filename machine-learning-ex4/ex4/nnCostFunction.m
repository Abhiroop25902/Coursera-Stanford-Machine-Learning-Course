...
%Changing Given y to required newY
for i=1:size(y,1)
	newY(i,y(i)) = 1;
end;
...
%Cost Funtion
thetaSum = sum(sum(Theta1(:,2:end).^2)) +  sum(sum(Theta2(:,2:end).^2)); %Sum of Theata except Biased Ones
J = -1/m * sum(sum(newY.*log(h) + (1-newY).*log(1-h))) + lambda/(2*m)* thetaSum;
